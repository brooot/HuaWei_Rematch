...
uint ids1[2000001];
uint ids2[2000001];
double transfer_amount[2000001];

uint* ids1_ptr=ids1;
uint* ids2_ptr=ids2;
double* money_ptr=transfer_amount;
...
void dfs1(int root, int current,  int depth, double preMoney)
{
    ...
    if (depth < 7)
    {
        ...
        for (; it!=NULL; it=it->next)
        {
            ...
            if (depth >= 2) {
                ...
                factor1 = it->money / preMoney;
                if (factor1 > 3 || factor1 < 0.2){
                    continue;
                }
            }
            ...
            if (depth >= 2 && lastPoint1[it->id2] == root)
            {
                ...
                factor1 = it_next1->money / it->money;
                if (factor1 > 3 || factor1 < 0.2) continue;
                //判断路径:it->it_next1->root_next1
                factor1 = root_next1->money / it_next1->money;
                if (factor1 > 3 || factor1 < 0.2) continue;
            ...
            current_node1 = it;
            dfs1(root, it->id2, depth + 1, it->money);
        }
    }
    inPath1[current] = false;
    path_pointer1--;
}
void dfs2(int root, int current,  int depth, double preMoney)
{
    ...
    if (depth < 7)
    {
        ...
        for (;it != NULL; it=it->next)
        {
            ...
            if (depth >= 2 && lastPoint2[it->id2] == root)
            {
                ...
                factor2 = it_next2->money / it->money;
                if (factor2 > 3 || factor2 < 0.2) continue;
                //判断路径:it->it_next2->root_next2
                factor2 = root_next2->money / it_next2->money;
                if (factor2 > 3 || factor2 < 0.2) continue;
            ...
            dfs2(root, it->id2, depth + 1, it->money);
        }
    }
...
void dfs3(int root, int current,  int depth, double preMoney)
{
    ...
    *path_pointer3++ = current;
    ...
    if (depth < 7)
    {
        ...
        for (;it != NULL; it=it->next)
        {
            ...
            dfs3(root, it->id2, depth + 1, it->money);
        }
    }
...
void dfs4(int root, int current, int depth, double preMoney)
{
    ...
    if (depth < 7)
    {
        ...
        for (; it != NULL; it = it->next)
        {
            ...
            if (depth >= 2 ){
                ...
                factor4 = (double)it->money / preMoney;
            ...
            if (depth >= 2 && lastPoint4[it->id2] == root)
            {
                ...
                factor4 = it_next4->money / it->money;
                if (factor4 > 3 || factor4 < 0.2) continue;
                // 判断路径:it->it_next4->root_next4
                factor4 = root_next4->money / it_next4->money;
                if (factor4 > 3 || factor4 < 0.2) continue;
            ...
            dfs4(root, it->id2, depth + 1, it->money);
        }
    }
...
void sort4(uint*  begin, uint* end)
{
    while (begin != end){
...
void search_3(uint* begin, uint* end)
{
    while (begin != end)
    {
        ...
        dfs3(current, current, 1, 0);
    }
}
void search_4(uint* begin, uint* end)
{
    while (begin != end)
    {
        int current = idHash[*begin++];
        markNeighborhoods4(current, current, 1);
        dfs4(current, current, 1, 0);
    }
}
...
int main()
{
    ...
    for (int i = 1;i <= nodeCount;i++) {  
		idHash[valid_ids[i-1]] = i;
        id_is_recorded[i] = true;
    }

    // cout << "id:" << 139 << "hash -> " << idHash[139] << endl;
    // cout << "id:" << 154 << "hash -> " << idHash[154] << endl;
    // cout << "id:" << 173 << "hash -> " << idHash[173] << endl;
    ...
    thread r4(sort4, seg3, last_valid_id_ptr);
    r1.join();
    r2.join();
    r3.join();
    r4.join();
    ...
    thread s4(search_4, seg3, last_valid_id_ptr);
    s1.join();
    s2.join();
    s3.join();
    s4.join();

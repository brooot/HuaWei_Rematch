    ...
    while (begin != end){
		...
		while(id_ptr->next!=NULL){
			if(!id_is_recorded[id_ptr->next->id2]){
                if(t){
                    cout << "delete" << valid_ids[id_ptr->next->id2] << endl;
                }
				EDGE* q=id_ptr->next;
...
int main()
{
    int read_fd = open("test_data_9153.txt", O_RDONLY, 0666);

    struct stat st;
    stat("test_data_9153.txt", &st);
    ...
    cout << "创建图完成" << endl;
    EDGE* temp = Graph[idHash[139]].next;
    cout << "139: ";
    while(temp!=NULL){
    	cout << valid_ids[temp->id2] << " ";
	temp = temp->next;
    }
    cout << endl;

    cout << "154: ";
     temp = Graph[idHash[154]].next;
    while(temp!=NULL){
    	cout << valid_ids[temp->id2] << " ";
	temp = temp->next;
    }
    ...
    cout << "154 ? " << valid_ids[idHash[154]] << endl;
